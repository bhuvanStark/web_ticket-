// Sales & Back-Office Roles V1 — Admin management page for the Back-Office
// roster (plan §5). Identical shape to SalesPage.jsx (see its file header
// for the rationale behind the self-contained, non-AppContext pattern);
// kept as a separate file rather than parameterizing one over both roles,
...
const BackOfficeFormModal = ({ mode, employee, onClose, onSaved, showToast }) => {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (isEdit) await updateBackOfficeInApi(employee.id, payload);
      else await createBackOfficeInApi(payload);
      showToast(`Back-Office employee ${isEdit ? 'updated' : 'added'} successfully.`, 'success');
      onSaved();
    } catch (err) {
      ...
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl border border-[#E4E7EC] overflow-hidden flex flex-col max-h-[90vh]">
        <div className="p-5 border-b border-[#E4E7EC] bg-[#F8FAFC] flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h3 className="font-extrabold text-lg text-[#172033]">{isEdit ? 'Edit Back-Office Employee' : 'Add Back-Office Employee'}</h3>
          </div>
...
export const BackOfficePage = () => {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const data = await fetchBackOfficeInApi();
      setEmployees(Array.isArray(data) ? data : []);
    } catch (err) {
      showToast?.(err.message || 'Could not load Back-Office roster', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-[#172033] tracking-tight">Back-Office Roster</h2>
          <p className="text-xs md:text-sm text-[#667085] mt-0.5">Manage Back-Office employees and their branch assignment.</p>
        </div>
        <button onClick={() => setModal({ mode: 'create' })} className="btn btn-primary shadow-sm text-xs font-bold">
          <Plus className="w-4 h-4" />
          <span>Add Back-Office Employee</span>
        </button>
      </div>
      ...
      <div className="card p-4 bg-white flex flex-col sm:flex-row sm:items-center justify-between gap-4 border border-[#E4E7EC] shadow-xs">
        ...
        <div className="text-xs font-bold text-[#004898] bg-[#EFF5FC] px-3 py-1.5 rounded-lg border border-[#B3D1F2] shrink-0">
          Total Back-Office Employees: {filtered.length}
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs text-left border-collapse bg-white">
            ...
            <tbody className="divide-y divide-[#F2F4F7]">
              {filtered.length === 0 ? (
                <tr><td colSpan="4" className="p-8 text-center text-[#667085]">No Back-Office employees match the selected filters.</td></tr>
              ) : (
                filtered.map((e) => (
